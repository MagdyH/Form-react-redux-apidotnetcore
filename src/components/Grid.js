...
export const Grid = (props)=>{
        ...
        const dataPage =props.data.filter((a,index)=>[index] >= startIndex && [index] < endIndex);// props.data.filter((a) => a.userId > startIndex && a.userId <= endIndex);

        return <table className="table table-bordered">
            <GridHeader />
            <tbody>
                {dataPage.map(user => <GridRow id={user.userId} key={user.userId} actions={props.actions} isEdit={false} api={props.api} {...user} />)}
            </tbody>
        </table>
}
...
export class GridRow extends React.Component {
    ...
    handleEditdata(e, obj) {
        this.setState({ isEdit: true });     
    }

    handleDeletedata(obj, e) {
    ...
    render() {
        return this.state.isEdit === true ? <EditRowGrid obj={this.props} id={this.props.userId} api={this.props.api} isEdit={this.state.isEdit} actions={this.props.actions} />  : <tr id={this.props.userId} key={this.props.userId}>
            ...
            <td>
                ...
                <button className='btn btn-danger'  onClick={()=>{this.setState({ show: true })}}>
...
export class EditRowGrid extends React.Component {
    constructor(props) {
        super(props)
